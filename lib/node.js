...
Node.prototype.checkWeb3Connection = function () {
    var self = this;
    console.log('web3.eth.net.isListening()', web3.eth.net.isListening())
    if (!this._web3) {
...
Node.prototype.setFilters = function () {
    ...
    try {
        web3.eth.subscribe('newBlockHeaders', function (err, hash) {
            var now = _.now();
            var time = now - self._lastChainLog;
            self._lastChainLog = now;
            console.log('newBlockHeaders', hash)
            if (hash === null) {
    ...
    try {
        web3.eth.subscribe('pendingTransactions', function (err, hash) {
            ...
            console.log('pendingTransactions', hash)
